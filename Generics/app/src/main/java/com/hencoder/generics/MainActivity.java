...
public class MainActivity<T> extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Class<? super List<String>> rawType = listToken.getRawType();
        ...
        // covariant 协变
        List<? extends View> objects = new ArrayList<TextView>();
        // contravariant 逆变/反变
        ...
        // 数组支持协变，而泛型不支持协变，所以一般不推荐泛型数组
        Shop<Apple>[] shops1 = new Shop<Apple>[10];
        Shop<?>[] shops2 = new Shop<?>[10];
        // 推荐使用List，其实除了基本类型用数组性能比较好之外，一般推荐用List代替数组
        List<Shop<Apple>> appleShops = new ArrayList<Shop<Apple>>();

        // 无法解决，T是未知的，不一定有这种构造方法
        T instance = new T();
        // 无法确定数组内存大小，无法创建泛型的数组
        T[] array = new T[]{};
        // 因此一般用Object数组，参考ArrayList内部实现
